...
JTC.config = {
    ...
    dayBgColor : [ '#EDECF0', '#3AA7FC', '#0891FC','#EDECF0'],
    dayColor : [ '#303136', '#303136', '#303136', '#999999'],
    
    forward : 1 ,   //控制日期可选择的范围  0:无限制;  1:仅可选择过去的日期; 2.仅可选择未来的日期
...
JTC.dialog = function(){
    ...
    this.getYMSelect = function(){
        ...
        var html = '<span id="'+ JTC.config.yearSpan +'" title="点击这里选择年份" style="width:80px;cursor:pointer;" onclick="JTC.events.beginToSelectYear()">2010</span>';
    ...
    this.getHeadPanel = function(){
        ...
        cell.title = '往前翻 月';
        cell.innerHTML = '<input type="button" value="<" style="width:20px; height:20px;border:1px solid #CCC;background-color:#EAEAEA;margin:2px;" onclick="JTC.events.turnTheMonth(-1)" />';
    ...
    this.getWeekPanel = function(){
        ...
        for(var i = 0; i< 7; i++)
        {
            ...
            cell.style.borderTop = '1px solid #666';
    ...
    this.getBottomPanel = function()
    {
        ...
        cell.style.textAlign = 'center';
        cell.style.borderTop = '1px solid #666';
        cell.innerHTML = '<input type="button"  value="今天" class="gm_t1_btn_alt" style="margin:2px!important;" onclick="JTC.events.resetToToday()" />';
        
        return table;
    };
    
    var load = JTC.util.createTable().table;
    load.setAttribute('style','width:210px;height:160px;font-size:12px;background-color:#ffffff; position:absolute;border:1px solid #666;border-radius: 3px;-moz-border-radius: 3px;margin-top:2px;');
    load.style.cssText = 'width:210px;height:160px;font-size:12px;background-color:#ffffff; position:absolute;border:1px solid #666;border-radius: 3px;-moz-border-radius: 3px;margin-top:2px;'; 
    load.border = 0;
    ...
    div.setAttribute('style','position:absolute; width:212; height:166;z-index:20000; font-size:12px; background-color:lightgreen;display:none;');
    div.style.cssText = 'position:absolute; width:212; height:166;z-index:20000; font-size:12px; background-color:lightgreen;display:none;';
...
JTC.events = {
    ...
    createDay : function(yy, mm)
    {
        ...
        for(var i=0; i< 37; i++)
        {
            ...
            obj.onclick = function(){
                var cur = this.style.cursor;
                if(cur == 'pointer'){
                    JTC.events.tdDayClick(this.innerHTML,0,0);
                }
            };
        }  
    },
    ...
    resetToToday : function()
    {
        ...
        JTC.events.tdDayClick(day,year,month);
    },
    ...
    tdDayClick : function(num,y,m)
    {
        var year = y==0? parseInt(JTC.$(JTC.config.yearSpan).innerHTML.match(/\d+/g)):y ;
        var month = m==0? parseInt(JTC.$(JTC.config.monthSpan).innerHTML.match(/\d+/g)):m;
